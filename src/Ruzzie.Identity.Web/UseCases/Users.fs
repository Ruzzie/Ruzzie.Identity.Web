...
module Users =
    ...
    let userExist email (repository: IUserRepository) =
        ...
        with ex ->
            createInvalidErrorWithExn "email" "userExists.unexpectedError" ex

    let userExists2 repository email =
        userExist email repository .=> (fun exists -> Ok(exists , email))
    ...
    let insertNewUser (repository: IUserRepository) userEntity =
        try
            ...
            if(createdEntity = null) then
                ...
                Ok(createdEntity)
        with ex ->
            createInvalidErrorWithExn "email" "insertNewUser.unexpectedError" ex
    ...
    let getUserByEmail (repository: IUserRepository) email =
        try
            ...
            if (not (userEntity = null)) then Ok(userEntity)
            else createInvalidError "email" ("getUserByEmail.doesNotExist" :: [])
        with ex ->
            createInvalidErrorWithExn "email" "getUserByEmail.unexpectedError" ex

    let getUserByEmailStr (repository: IUserRepository) emailStr =
        try
            let userEntity = repository.GetUserByEmail(emailStr)
            if (not (userEntity = null)) then Ok(userEntity)
            else createInvalidError "email" ("getUserByEmailStr.doesNotExist" :: [])
        with ex ->
            createInvalidErrorWithExn "email" "getUserByEmailStr.unexpectedError" ex
    ...
    let updateUser (repository: IUserRepository) utcNow userEntity =
        try
            ...
            if(createdEntity = null) then
                ...
                Ok(createdEntity)
        with ex ->
            createInvalidErrorWithExn "email" "updateUser.unexpectedError" ex
    ...
    let deleteUser (repository: IUserRepository) userId =
        try
            ...
            Ok(ignore true)
        with ex ->
            createInvalidErrorWithExn "email" "deleteUser.unexpectedError" ex
    ...
    let createPasswordResetToken utcNow encrypt email =
        ...
        with ex ->
            createInvalidErrorWithExn "email" "passwordResetToken.unexpectedError" ex
    ...
    let createEmailValidationToken utcNow encrypt (req:RegisterUserRequest) =
        ...
        with ex ->
            createInvalidErrorWithExn "email" "emailValidationToken.unexpectedError" ex
    ...
    let createNewUser utcNow withEmailActivation repository encrypt (req: RegisterUserRequest) =
        ...
        let emailValidationToken =
            (if (withEmailActivation) then Result.map Some (createEmailValidationToken utcNow encrypt req)
             else Ok(None))
        ...
        let entityResult =
             (fun (pwd, withValidationTokenOption) ->
                ...
                | Some validationToken ->
                    UserRegistration
                        ((EmailAddressValue.value req.Email), pwd,
                         StringRequiredValue.value req.Firstname, StringRequiredValue.value req.Lastname, validationToken,
                         utcNow, Nullable utcNow, AccountValidationStatus.Pending |> int, Nullable utcNow)
                | None ->
                    UserRegistration
                        ((EmailAddressValue.value req.Email), pwd,
                         StringRequiredValue.value req.Firstname, StringRequiredValue.value req.Lastname, String.Empty,
                         utcNow, Nullable utcNow, AccountValidationStatus.None |> int, Nullable utcNow))
             <!> tokenAndPasswordRes

        entityResult .=> insertNewUser repository

    let createRegisterUserResponse jwtSecretKey utcNow (entity: UserRegistration) =
        ...
        Result.map (fun jwt ->
            { RegisterUserResp.email = entity.Email
              ...
              JWT = jwt }, entity.EmailValidationToken) jwtToken

    let createAuthenticateUserResponse jwtSecretKey utcNow (entity: UserRegistration) =
    ...
    let registerUser
        utcNow repository encryptFunc jwtSecretKey
        withEmailActivation emailService emailTemplates buildActivationUrlFunc (logger:ILogger) (req:RegisterUserRequest) =

        let errorWhenUserNotExists x exists =
            if (exists) then createInvalidError "email" ("userAlreadyExists" :: [])
            else Ok(x)

        let responseRes =
            (userExist req.Email) >=> errorWhenUserNotExists req
        ...
        | Ok(response, emailToken) ->
            if (withEmailActivation) then
                ...
                let createEmailResult = (emailTemplates.CreateUserRegistrationActivationMail response.email (buildActivationUrlFunc emailToken))
                let createAndSendResult = createEmailResult.AndThen (fun emailToSend -> toErrorKindResult (sendEmail emailService emailToSend))

                match createAndSendResult with
                |Ok statusCode -> Ok({response with witActivationMail = true; activationMailResult = statusCode.ToString()})
                |Error errList ->
    ...
    let authenticateLoginUser utcNow repository jwtSecretKey (req: AuthenticateUserRequest) =
        ...
        let errorWhenUserNotExists x exists =
            if not(exists) then createInvalidError "password" ([])
            else Ok(x)

        let responseRes =
            (userExist req.Email) >=> errorWhenUserNotExists req.Email >=> getUserByEmail repository
        ...
        | Ok(isValidPassword, entity) ->
            ...
            else createInvalidError "password" ([])
        | Error e -> Error e
    ...
    let forgotPassword utcNow repository encryptFunc emailService emailTemplates forgotPasswordUrlFunc (logger:ILogger) email =

        let createSendEmailRequest token = emailTemplates.CreateUserPasswordForgetMail (EmailAddressValue.value email) (forgotPasswordUrlFunc token)
        ...
        |Ok userExists ->
            if not(userExists) then
                ...
                let runResult = (toListOfError createTokenResult)
                                      .=> (fun tokenStr ->  toListOfError(updateUserWithPasswordResetToken utcNow repository email tokenStr))
                            ...
                            |ValidToken tokenInfoFromDb ->
                                if tokenInfoFromReq = tokenInfoFromDb then
                                    ...
                                    let passwordValidationResult =
                                            DomainTypes.PasswordValue.create req.NewPasswordInput
                                                (Some
                                                    { DomainTypes.ErrInfo.FieldName = Some "newPassword"
                                                      DomainTypes.ErrInfo.Details = None }) userEntity.Email userEntity.Firstname userEntity.Lastname

                                    let newHashedPasswordRes = passwordValidationResult
    ...
    let organisationExists orgName (repository: IOrganisationRepository) =
        ...
        with ex ->
            createInvalidErrorWithExn "OrganisationName" "organisationExists.unexpectedError" ex
    ...
    let getOrganisationById (repository:IOrganisationRepository) orgId =
        try
            ...
            if (not (entity = null)) then Ok(entity)
            else createInvalidError "organisationId" ("getOrganisationById.doesNotExist" :: [])
        with ex ->
            createInvalidErrorWithExn "organisationId" "getOrganisationById.unexpectedError" ex
    ...
    let insertNewOrganisation (repository:IOrganisationRepository) orgEntity =
        try
            ...
            if(createdEntity = null) then
                ...
                Ok(createdEntity)
        with ex ->
            createInvalidErrorWithExn "organisationName" "insertNewOrganisation.unexpectedError" ex
    ...
    let addUserToOrganisation utcNow (repository:IOrganisationRepository) userId role (orgEntity:Organisation) =
         try
            ...
            if(orgsForUser = null) then
                createInvalidError "organisationName" ("addUserToOrganisation.getOrganisationsForUser.errorGotNullResult"::[])
            else
                Ok (orgEntity, orgsForUser)
         with ex ->
            createInvalidErrorWithExn "organisationName" "addUserToOrganisation.unexpectedError" ex
    ...
    let createUserOrganisationsApiTypeUnsafe userOrgsEntities getOrganisation = [
            ...
            for (userOrg:UserOrganisation) in userOrgsEntities do
                let (orgEntityForCurrent:Organisation) = getOrganisation userOrg.RowKey
                {
                    ...
                    ApiTypes.UserOrganisation.userJoinedTimestamp = userOrg.JoinedCreationDateTimeUtc.ToUnixTimeMilliseconds()
                    ApiTypes.UserOrganisation.name = orgEntityForCurrent.OrganisationName
                    ApiTypes.UserOrganisation.createdTimestamp = orgEntityForCurrent.CreationDateTimeUtc.ToUnixTimeMilliseconds()
                    ApiTypes.UserOrganisation.lastModifiedTimestamp = orgEntityForCurrent.LastModifiedDateTimeUtc.ToUnixTimeMilliseconds()
    ...
    let createUserApiTypeUnsafe (userRegistrationEntity:UserRegistration) userOrganisationsApiType =
                {
                    ApiTypes.User.id = userRegistrationEntity.RowKey
                    ApiTypes.User.email = userRegistrationEntity.Email
                    ApiTypes.User.firstname = userRegistrationEntity.Firstname
                    ApiTypes.User.lastname = userRegistrationEntity.Lastname
                    ApiTypes.User.createdTimestamp = userRegistrationEntity.CreationDateTimeUtc.ToUnixTimeMilliseconds()
                    ApiTypes.User.lastModifiedTimestamp = userRegistrationEntity.LastModifiedDateTimeUtc.ToUnixTimeMilliseconds()
                    ApiTypes.User.accountValidationStatus = enum<AccountValidationStatus> userRegistrationEntity.AccountValidationStatus
                    ApiTypes.User.organisations = userOrganisationsApiType
                }

    let createUserApiTypeForCreateOrg (userRegistrationEntity:UserRegistration) (initialOrgEntity:Organisation) (userOrgsEntities) (orgRepo:IOrganisationRepository) =
        try
            ...
            Ok userApiType
        with ex ->
            createInvalidErrorWithExn "organisationName" "createUserApiTypeForCreateOrg.unexpectedError" ex

    let createOrganisationForUser utcNow userRepository (orgRepository:IOrganisationRepository) (req:AddOrganisationRequest) =
        ...
        .=> (fun(userRegistration, orgExists) ->
                if orgExists then
                    ...
                    let orgEntity = Organisation(StringRequiredValue.value req.OrganisationName, EmailAddressValue.value req.UserId, utcNow)
    ...
    let addUserToOrganisationWithRole utcNow userRepository (orgRepository:IOrganisationRepository) userId organisationId role =
    ...
    let getUserInfo utcNow userRepository (orgRepository:IOrganisationRepository) userId =
        ...
        .=> (fun userEntity ->
             ...
             with ex ->
                createInvalidErrorWithExn "userId" "getUserInfo.unexpectedError" ex
            )

    let updateUserInfo utcNow (userRepository:IUserRepository) (orgRepository:IOrganisationRepository) (req:UpdateUserRequest) =
        getUserByEmail userRepository req.UserId
        .=> (fun userEntity ->
                try
                    ...
                    Ok (createUserApiTypeUnsafe userEntity orgsApiType)
                with ex ->
                    createInvalidErrorWithExn "userId" "updateUserInfo.unexpectedError" ex
            )
