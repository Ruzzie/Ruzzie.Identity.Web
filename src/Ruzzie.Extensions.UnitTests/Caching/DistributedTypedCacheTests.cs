...
{
    private DistributedTypedCache<MemoryDistributedCache> _cache = null!;
    ...
    {
        var optionsFactory =
            new OptionsFactory<MemoryDistributedCacheOptions>(new List<IConfigureOptions<
                                                                  MemoryDistributedCacheOptions>>()
                                                            , new List<IPostConfigureOptions<
                                                                  MemoryDistributedCacheOptions>>());
        var options                 = new OptionsManager<MemoryDistributedCacheOptions>(optionsFactory);
        var distributedCacheForTest = new MemoryDistributedCache(options);

        _cache = new DistributedTypedCache<MemoryDistributedCache>(distributedCacheForTest
    ...
    {
        //Arrange
        var entryToAdd = new DataToCache("Org", "SetGetPropertyTest", DateTimeOffset.UtcNow, DateTimeOffset.UtcNow);
    ...
    {
        //Arrange
        var entryToAdd = new DataToCache("Org"
                                       , "SetGetAsyncPropertyTest"
    ...
    [Test]
    public unsafe void GetOrAddDelegatePtr()
    {
        //Act
        var entryFromCache = _cache.GetOrAdd(nameof(GetOrAddAddsSuccess), &Add);

        //Assert
        entryFromCache.Should().BeEquivalentTo(Add(""));
